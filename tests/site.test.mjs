...
test("exports the complete Slovak LNV12 site", async () => {
  ...
  assert.match(html, /4 080 km/);
  assert.match(html, /Michal Drieňovský/);
  assert.match(html, /Martin Bajla/);
  assert.match(html, /Ondrej Jančovič/);
  assert.match(html, /Miroslav Nemethy/);
  assert.match(html, /3\s667,5(?:<!-- -->)? km/);
  assert.match(html, /412,5(?:<!-- -->)? km/);
  assert.match(html, /Pavol Uhlár/);
  assert.match(html, /Andrea Škorcová/);
